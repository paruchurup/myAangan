...
export class ServiceDirectoryService {
  ...
  getProviders(categoryId?: number, search?: string): Observable<ApiResponse<ProviderSummary[]>> {
  ...
  submitReview(providerId: number, req: ReviewRequest): Observable<ApiResponse<Review>> {
    return this.http.post<ApiResponse<Review>>(
      `${this.base}/providers/${providerId}/reviews`, req);
  }

  deleteReview(providerId: number, reviewId: number): Observable<ApiResponse<void>> {
    return this.http.delete<ApiResponse<void>>(
      `${this.base}/providers/${providerId}/reviews/${reviewId}`);
  }
}
